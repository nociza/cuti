...
RUN { \
        printf '%s\\n' \
            ...
            'fi' \
            'if [ ! -x "$HERMES_CLI" ]; then' \
            '    echo "Hermes Agent CLI not found. Enable the hermes provider or run: /usr/local/bin/cuti-install-hermes" >&2' \
            '    exit 1' \
            'fi' \
            'exec "$HERMES_CLI" "$@"'; \
...
'''
    ...
    DEVCONTAINER_JSON_TEMPLATE = {
        ...
        "build": {
            ...
            "context": ".",
            "args": {"USERNAME": "cuti", "USER_UID": "1000", "USER_GID": "1000"},
        },
        "runArgs": ["--init"],
        "containerEnv": {
    ...
    def __init__(
        ...
        *,
        ...
        # Host-tool availability is probed lazily on first access so that
        # constructing the service never shells out (keeps it testable and
        # side-effect-free). See the docker_available/colima_available properties.
        self._docker_available: bool | None = None
        self._colima_available: bool | None = None

    @property
    def docker_available(self) -> bool:
        """Whether the ``docker`` CLI is available (probed lazily, cached)."""
        if self._docker_available is None:
            self._docker_available = self._check_tool_available("docker")
    ...
    @property
    def colima_available(self) -> bool:
        """Whether the ``colima`` CLI is available (probed lazily, cached)."""
        if self._colima_available is None:
            self._colima_available = self._check_tool_available("colima")
        return self._colima_available

    @colima_available.setter
    def colima_available(self, value: bool) -> None:
        self._colima_available = value
        ...
        try:
            if show_output:
                ...
                return subprocess.CompletedProcess(
                    cmd, process.returncode, stdout=''.join(output), stderr=None
                )
            else:
    ...
    def _prepare_hermes_storage(self) -> Path:
        """Ensure Hermes Agent state storage exists on the host."""
        ...
        for relative in [
            ...
            (hermes_dir / relative).mkdir(parents=True, exist_ok=True)
        (hermes_dir / ".env").touch(exist_ok=True)
        return hermes_dir
    ...
    def _prepare_cloud_provider_mounts(
        ...
        if providers & {"codex", "opencode", "openclaw", "hermes"}:
        ...
        if "opencode" in providers:
            opencode_home, opencode_config, opencode_data = (
                self._prepare_opencode_storage()
            )
            mount_args.extend(
                [
                    "-v",
        ...
        elif "hermes" in providers:
            openclaw_dir = Path.home() / ".openclaw"
            if openclaw_dir.exists():
                mount_args.extend(["-v", f"{openclaw_dir}:/home/cuti/.openclaw:ro"])

        if "hermes" in providers:
            hermes_dir = self._prepare_hermes_storage()
            mount_args.extend(["-v", f"{hermes_dir}:/home/cuti/.hermes:rw"])
            claude_dir = Path.home() / ".claude"
            if claude_dir.exists():
                mount_args.extend(["-v", f"{claude_dir}:/home/cuti/.claude:ro"])
    ...
    def _check_tool_available(self, tool: str) -> bool:
        ...
        try:
            ...
            # OSError covers FileNotFoundError (tool absent) and PermissionError
            # (exec denied, e.g. in restricted sandboxes).
            return False
    ...
    def _install_with_brew(self, package: str) -> bool:
        ...
        result = self._run_command(
            ["brew", "install", package], timeout=300, show_output=True
        )
        ...
        if self.is_macos:
            ...
            if not self._check_tool_available("brew"):
                if self._prompt_install("Homebrew", "Official install script"):
                    ...
                    result = self._run_command(
                        install_cmd.split(), timeout=600, show_output=True
                    )
                    if result.returncode != 0:
                        return False
                else:
    ...
    def _generate_dockerfile(self, project_type: str) -> str:
        ...
        if (self.working_dir / "src" / "cuti").exists() and (
...
'''
        ...
        if tools_setup:
            dockerfile = dockerfile.replace(
                "# Set the default command", tools_setup + "\n# Set the default command"
            )
        ...
        for attempt in range(max_retries):
            try:
                ...
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    result = self._run_command(
                        build_cmd, timeout=1800, show_output=True
                    )
                    if result.returncode == 0:
                        ...
                        error_output = str(result.stderr or "") + str(
                            result.stdout or ""
                        )
                        if any(
                            ...
                            if attempt < max_retries - 1:
                                print(
                                    f"⚠️  Build failed due to connection issue. Retrying in {retry_delay} seconds... (attempt {attempt + 2}/{max_retries})"
                                )
                ...
                if attempt < max_retries - 1:
                    print(
                        f"⚠️  Build failed with error: {e}. Retrying... (attempt {attempt + 1}/{max_retries})"
                    )
                    time.sleep(retry_delay)
                    continue
                else:
                    print(
                        f"❌ Container build failed after {max_retries} attempts: {e}"
                    )
                    return False
    ...
    def generate_devcontainer(self, project_type: str | None = None) -> bool:
        ...
        devcontainer_json_path.write_text(
            json.dumps(self.DEVCONTAINER_JSON_TEMPLATE, indent=2)
        )
    ...
    def _detect_project_type(self) -> str:
        ...
        if (self.working_dir / "package.json").exists():
            return (
                "javascript"
                if not (self.working_dir / "pyproject.toml").exists()
                else "fullstack"
            )
        elif (self.working_dir / "pyproject.toml").exists() or (
            self.working_dir / "requirements.txt"
        ).exists():
            return "python"
    ...
    def run_provider_update(
        ...
        *,
        ...
        for active_index, container_id in enumerate(active_container_ids, start=1):
            ...
            if exit_code != 0:
                print(
                    f"❌ Failed to update {provider} in active container {container_id}"
                )
                return exit_code
    ...
    def run_in_container(
        ...
        *,
        ...
        Args:
            ...
            mount_docker_socket: Mount the host Docker socket into the container.
                Off by default: the socket is root-equivalent on the host, and the
                container runs the agent with skip-permissions, so mounting it turns
                a prompt injection into host root. Opt in with ``--docker-socket``
                only when you explicitly need Docker-in-Docker.
        ...
        if self.is_macos:
            ...
            if (
                colima_status.returncode == 0
                and "running" in colima_status.stdout.lower()
            ):
                print("🐳 Using Colima runtime")
                mount_options = "rw"  # Colima usually handles basic rw well
            else:
                print(
                    "🐳 Using Docker Desktop - trying cached mode for better macOS compatibility"
                )
        ...
        docker_args = [
            ...
            # Shadow saved-account credentials with an empty tmpfs so the
            # (skip-permissions) agent cannot read OAuth tokens / API keys for
            # accounts other than the active one. The active account is provided
            # separately via the .claude-linux mount below.
            "--tmpfs",
            "/home/cuti/.cuti-shared/claude-accounts:rw,noexec,nosuid",
            "--label",
...
if cuti_provider_selected hermes; then
    export HERMES_HOME=/home/cuti/.hermes
    export HERMES_INSTALL_DIR=/home/cuti/.hermes/hermes-agent
fi
...
if cuti_provider_selected hermes; then
    mkdir -p \
        ...
        /home/cuti/.hermes/image_cache \
        /home/cuti/.hermes/audio_cache \
        /home/cuti/.hermes/whatsapp/session \
        /home/cuti/.agents/skills 2>/dev/null || true
    touch /home/cuti/.hermes/.env 2>/dev/null || true
    sudo chown -R cuti:cuti /home/cuti/.hermes 2>/dev/null || true

    if [ -x /home/cuti/.hermes/hermes-agent/venv/bin/hermes ]; then
        echo "✅ Hermes Agent CLI already installed"
    else
        echo "☤ Installing Hermes Agent CLI..."
        export HOME=/home/cuti
        export HERMES_HOME=/home/cuti/.hermes
        export HERMES_INSTALL_DIR=/home/cuti/.hermes/hermes-agent
        if /usr/local/bin/cuti-install-hermes > /tmp/hermes-install.log 2>&1; then
            echo "✅ Hermes Agent CLI installed"
            hash -r 2>/dev/null || true
        else
            echo "⚠️  Failed to install Hermes Agent CLI"
            cat /tmp/hermes-install.log
        fi
    fi
...
if cuti_provider_selected hermes; then
    if [ -f /home/cuti/.hermes/.env ] && grep -Eq '^[[:space:]]*[A-Z0-9_]*(API_KEY|_TOKEN|_KEY)[[:space:]]*=[[:space:]]*[^[:space:]#]+' /home/cuti/.hermes/.env; then
        echo "🔑 Found Hermes provider credentials in /home/cuti/.hermes/.env"
    elif [ -f /home/cuti/.claude/.credentials.json ] || [ -f /home/cuti/.claude-linux/.credentials.json ]; then
        echo "🔑 Found Claude credentials that Hermes can use for native Anthropic auth"
    else
        echo "⚠️  No Hermes provider credentials found. Configure once with: hermes setup"
        echo "   Your Hermes config, memory, skills, gateway state, and sessions persist under ~/.hermes."
    fi

    if command -v hermes > /dev/null 2>&1; then
        echo "✅ Hermes Agent CLI is available at: $(command -v hermes)"
        if hermes version > /dev/null 2>&1 || hermes --version > /dev/null 2>&1; then
            echo "✅ Hermes Agent CLI verified"
        else
            echo "⚠️  Hermes Agent CLI found but version check failed"
        fi
    else
        echo "❌ Hermes Agent CLI not found in PATH!"
        echo "   Expected wrapper at /usr/local/bin/hermes"
    fi
fi
...
if [ -S /var/run/docker.sock ]; then
    ...
    sudo chmod 660 /var/run/docker.sock 2>/dev/null || true  # group-rw (user is in docker group); not world-writable
...
"""
    ...
    def clean(self, clean_credentials: bool = False) -> bool:
        ...
        if clean_credentials:
            ...
            print(
                "💡 Tip: Linux Claude credentials preserved. Use --clean-credentials to remove them."
            )
