...
class ContainerUsageSync:
    ...
    def sync_usage_data(self) -> int:
        ...
        try:
                    ...
                    try:
                        ...
                        if isinstance(usage_data, list):
                            for record in usage_data:
                                if self._insert_usage_record(
                                    cursor, record, str(project_dir)
                                ):
                                    records_synced += 1
                        elif isinstance(usage_data, dict):
                            if self._insert_usage_record(
                                cursor, usage_data, str(project_dir)
                            ):
                                records_synced += 1
            ...
            if claude_monitor_path.exists():
                records_synced += self._sync_claude_monitor_data(
                    cursor, claude_monitor_path
                )
            ...
            if records_synced > 0:
                logger.info(
                    f"Synced {records_synced} usage records from container to host"
                )
    ...
    def _insert_usage_record(
        self, cursor, record: dict[str, Any], project_path: str
    ) -> bool:
