...
def _build_tools_section(tools: list[dict[str, Any]]) -> str:
    ...
    enabled_tools = [
        tool for tool in tools if tool.get("enabled") and tool.get("installed")
    ]
    if enabled_tools:
...
def update_instruction_files_with_tools(
    ...
    *,
    ...
    if instruction_files is not None:
        ...
        provider_files = ProviderManager(
            storage_dir=provider_storage_dir
    ...
    for filename in resolved_files:
        ...
        if section_start != -1:
            if section_end != -1:
                new_content = (
                    content[:section_start] + tools_content + content[section_end:]
                )
            else:
