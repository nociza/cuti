...
class ClaudeAccountManager:
    ...
    def __init__(self, storage_dir: str | None = None):
        ...
        # Credentials live here — keep the trees owner-only.
        for _secret_dir in (self.accounts_dir, self.active_dir):
            try:
    ...
    def _write_private(path: Path, content: str) -> None:
        """Atomically write a secret file as owner-only (0600).

        Creates the file with 0600 from the start (via os.open) so there is no
        window where the credentials are world/group-readable under the umask.
        """
    ...
    def _account_dir(self, name: str) -> Path:
        """Resolve an account's directory, rejecting unsafe names.

        The name is reduced to an allowlist of characters and the resolved
        path is asserted to stay inside ``accounts_dir``. This prevents a
        crafted name (e.g. ``../../etc``) from escaping the accounts directory
        for any read, write, or ``rmtree`` operation.
        """
        safe_name = "".join(
        ...
        account_dir = (accounts_root / safe_name).resolve()
        if account_dir != accounts_root and accounts_root not in account_dir.parents:
            raise ValueError(f"Account name escapes accounts directory: {name!r}")
        return account_dir
    ...
    def _save_metadata(self, metadata: dict[str, Any]):
        """Save accounts metadata (owner-only)."""
        metadata["last_updated"] = datetime.now().isoformat()
        self._write_private(self.metadata_file, json.dumps(metadata, indent=2))
    ...
    def list_accounts(self, include_backups: bool = False) -> list[dict[str, Any]]:
        ...
        for name, info in metadata.get("accounts", {}).items():
            ...
            account_dir = self._account_dir(name)
    ...
    def count_backup_accounts(self) -> int:
        ...
        return sum(
            1
            for name in metadata.get("accounts", {}).keys()
    ...
    def save_account(self, name: str) -> bool:
        ...
        if not creds_file.exists():
            raise ValueError(
                "No credentials found. Please authenticate with 'claude login' first."
            )

        # Create account directory (validated + containment-checked).
        account_dir = self._account_dir(name)
        ...
        for item in self.active_dir.iterdir():
            if item.is_file():
                ...
                if any(
                    ...
                    suffix in item.name
                    for suffix in [".backup", ".bak", ".old", ".corrupted"]
                ):
                    item.unlink()
    ...
    def get_account_info(self, name: str) -> dict[str, Any] | None:
    ...
    def save_api_key(
        ...
        if provider == "anthropic_api":
            ...
            if use_bearer_token:
                ...
                if not all([region, access_key_id, secret_access_key]):
                    raise ValueError(
                        "Bedrock with access keys requires region, access_key_id, and secret_access_key"
                    )
                api_keys["bedrock"] = {
                    "region": region,
        ...
        # Save API keys file with restricted permissions (owner-only from creation)
        self._write_private(api_key_file, json.dumps(api_keys, indent=2))
    ...
    def get_api_key(
        self, name: str, provider: str = "anthropic"
    ) -> dict[str, Any] | None:
    ...
    def get_env_vars(
        self, name: str, include_claude_code_vars: bool = True
    ) -> dict[str, str]:
        ...
        for provider in providers:
            ...
            if provider == "anthropic":
                ...
                if auth_method == "bearer_token":
                    ...
                    env_vars["AWS_BEARER_TOKEN_BEDROCK"] = api_key_info.get(
                        "bearer_token", ""
                    )
                    env_vars["AWS_REGION"] = region
                else:
                    # Option B: Access keys
                    env_vars["AWS_ACCESS_KEY_ID"] = api_key_info.get(
                        "access_key_id", ""
                    )
                    env_vars["AWS_SECRET_ACCESS_KEY"] = api_key_info.get(
                        "secret_access_key", ""
                    )
                    env_vars["AWS_REGION"] = region
    ...
    def generate_env_script(self, name: str, unset_first: bool = True) -> str:
        ...
        lines.append(f'echo "✓ Environment variables configured for account: {name}"')
    ...
    def save_env_script(self, name: str) -> Path | None:
