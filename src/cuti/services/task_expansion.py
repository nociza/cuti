...
class TaskExpansionEngine:
    ...
    def analyze_task_complexity(self, task_description: str) -> tuple[TaskComplexity, list[str]]:
        ...
        has_simple_indicator = any(indicator in task_lower for indicator in simple_indicators)
    ...
    def match_template(self, task_description: str, keywords: list[str]) -> str | None:
    ...
    def expand_task(self, task_description: str, context: dict[str, Any] | None = None) -> TaskBreakdown:
    ...
    def _resolve_execution_order(self, subtasks: list[SubTask]) -> list[str]:
        ...
        while remaining_tasks:
            ...
            if not ready_tasks:
                ...
                ready_tasks = list(remaining_tasks.items())

            # Sort by priority and add to order
            ready_tasks.sort(key=lambda x: x[1].priority)

            for task_id, _task in ready_tasks[:1]:  # Add one at a time
    ...
    def _identify_parallel_tasks(self, subtasks: list[SubTask]) -> list[list[str]]:
        ...
        for _level, task_ids in level_groups.items():
            if len(task_ids) > 1:
                parallel_groups.append(task_ids)

        return parallel_groups

    def _identify_risk_factors(self, complexity: TaskComplexity, template_name: str | None) -> list[str]:
    ...
    def _generate_success_metrics(self, task_description: str, subtasks: list[SubTask]) -> list[str]:
        ...
        categories = {task.category for task in subtasks}
    ...
    def save_task_breakdown(self, breakdown: TaskBreakdown, file_path: str | None = None) -> str:
    ...
    def load_task_breakdown(self, file_path: str) -> TaskBreakdown | None:
