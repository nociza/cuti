...
from ..core.todo_models import TodoItem, TodoList, TodoPriority, TodoSession, TodoStatus
...
class TodoService:
    ...
    def _ensure_master_list(self) -> None:
    ...
    def _sync_goal_file(self, master_list: TodoList) -> None:
    ...
    def save_goal_file(self, master_list: TodoList) -> None:
    ...
    def get_all_todos(self, limit: int | None = None) -> list[TodoItem]:
    ...
    def _save_list_with_conn(self, todo_list: TodoList, conn: sqlite3.Connection) -> str:
    ...
    def _row_to_list(self, row: sqlite3.Row, cursor: sqlite3.Cursor) -> TodoList:
    ...
    def _row_to_session(
        ...
        for list_row in cursor.fetchall():
            ...
            if session.master_list is None or sub_list.id != session.master_list.id:
                session.sub_lists.append(sub_list)

        return session

    def get_master_list(self) -> TodoList | None:
    ...
    def get_session(self, session_id: str) -> TodoSession | None:
    ...
    def get_active_session(self) -> TodoSession | None:
