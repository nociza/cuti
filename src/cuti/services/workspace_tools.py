...
class WorkspaceToolsManager:
    ...
    def get_workspace_config(self) -> dict[str, Any]:
        ...
        if self.workspace_tools_config.exists():
            try:
                with open(self.workspace_tools_config) as f:
                    return json.load(f)
            except Exception:
                pass
    ...
    def install_tool_for_workspace(
        self, tool_name: str, tool_config: dict[str, Any], scope: str = "workspace"
    ) -> dict[str, Any]:
        """
        ...
        if "npm install -g" in install_cmd:
            ...
            package_name = (
                install_cmd.split("@")[-1] if "@" in install_cmd else tool_name
            )
            local_install_cmd = (
                f"cd {install_dir.parent} && npm install {package_name} --prefix ."
            )
            ...
            if scope == "workspace":
                result["message"] = (
                    "APT packages cannot be installed at workspace scope. Use container or rebuild image."
                )
                return result
        ...
        try:
            ...
            if result_proc.returncode == 0:
                ...
                result["success"] = True
                result["message"] = (
                    f"Tool {tool_name} installed successfully in {scope}"
                )
            else:
    ...
    def _create_tool_wrapper(
        self, tool_name: str, tool_config: dict[str, Any], bin_dir: Path, lib_dir: Path
    ):
...
"""
    ...
    def list_workspace_tools(self) -> dict[str, Any]:
        ...
        for tool_name in config.get("enabled_tools", []):
            ...
            if tool_path.exists():
                tools["workspace_tools"][tool_name] = tools.get(
                    "workspace_tools", {}
                ).get(tool_name, {})
