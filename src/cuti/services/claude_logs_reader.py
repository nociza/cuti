...
class ClaudeLogsReader:
    ...
    def get_conversation_context(
        ...
        for prompt_id, prompt in user_prompts.items():
            exchange = {'user': prompt, 'assistant': assistant_responses.get(prompt_id)}
            conversation.append(exchange)

        # Sort by timestamp and limit
        conversation.sort(key=lambda x: x['user'].get('timestamp', ''), reverse=True)
