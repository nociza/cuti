...
from claude_monitor.core.plans import Plans, PlanType, get_cost_limit, get_token_limit
...
class ClaudeMonitorIntegration:
    ...
    def __init__(self, claude_data_path: str | None = None, plan_type: str = 'pro'):
        """
    ...
    def load_usage_data(
        self, hours_back: int | None = None, cost_mode: CostMode = CostMode.AUTO
    ) -> list[UsageEntry]:
        """
        ...
        try:
            entries, _ = load_usage_entries(
                data_path=self.claude_data_path, hours_back=hours_back, mode=cost_mode
            )
    ...
    def get_daily_usage(self, days: int = 30) -> list[DailyUsage]:
        ...
        try:
            ...
            aggregator = UsageAggregator(
                data_path=self.claude_data_path, aggregation_mode="daily"
            )
    ...
    def get_monthly_usage(self, months: int = 6) -> list[dict[str, Any]]:
        ...
        try:
            ...
            # Use claude_monitor's aggregator
            aggregator = UsageAggregator(
                data_path=self.claude_data_path, aggregation_mode="monthly"
            )
    ...
    def get_usage_trends(self, days: int = 7) -> dict[str, Any]:
        ...
        try:
            ...
            token_trend = (
                'increasing'
                if recent_tokens > earlier_tokens
                else 'decreasing' if recent_tokens < earlier_tokens else 'stable'
            )

            cost_trend = (
                'increasing'
                if recent_cost > earlier_cost
                else 'decreasing' if recent_cost < earlier_cost else 'stable'
            )

            request_trend = (
                'increasing'
                if recent_requests > earlier_requests
                else 'decreasing' if recent_requests < earlier_requests else 'stable'
            )
    ...
    def get_usage_predictions(self) -> dict[str, Any]:
        ...
        try:
            ...
            recent_entries = [
                e
                for e in entries
                if e.timestamp.replace(tzinfo=None)
                >= five_hours_ago.replace(tzinfo=None)
            ]
            ...
            if recent_entries:
                ...
                tokens_per_hour = (
                    current_tokens / hours_in_period if hours_in_period > 0 else 0
                )
                cost_per_hour = (
                    current_cost / hours_in_period if hours_in_period > 0 else 0
                )
            else:
                # Use 7-day average
                week_tokens = sum(
            ...
            hours_until_token_limit = (
                remaining_tokens / tokens_per_hour
                if tokens_per_hour > 0
                else float('inf')
            )
            hours_until_cost_limit = (
                remaining_cost / cost_per_hour if cost_per_hour > 0 else float('inf')
            )

            hours_until_limit = min(hours_until_token_limit, hours_until_cost_limit)
            limit_type = (
                'tokens' if hours_until_token_limit < hours_until_cost_limit else 'cost'
            )

            # Project usage for end of 5-hour block
            hours_left_in_block = max(
                0, 5 - min(5, (now - five_hours_ago).total_seconds() / 3600)
            )
            projected_tokens_block = current_tokens + (
                tokens_per_hour * hours_left_in_block
            )
            ...
            return {
                ...
                'current_usage': {
                    ...
                    'tokens_percentage': (
                        round((current_tokens / token_limit) * 100, 1)
                        if token_limit > 0
                        else 0
                    ),
                    'cost_percentage': (
                        round((current_cost / cost_limit) * 100, 1)
                        if cost_limit > 0
                        else 0
                    ),
                },
                ...
                'projections': {
                    ...
                    'hours_until_limit': (
                        round(hours_until_limit, 1)
                        if hours_until_limit != float('inf')
                        else None
                    ),
                    'limit_type': (
                        limit_type if hours_until_limit != float('inf') else None
                    ),
