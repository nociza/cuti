...
class GlobalDataManager:
    ...
    def __init__(self, global_dir: str | None = None):
        """
...
"""
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(str(self.db_path), timeout=30.0) as conn:
            ...
            cursor.execute(
                'CREATE INDEX IF NOT EXISTS idx_favorites_project ON favorite_prompts(project_path)'
            )
            cursor.execute(
                'CREATE INDEX IF NOT EXISTS idx_favorites_used ON favorite_prompts(last_used)'
            )
            cursor.execute(
                'CREATE INDEX IF NOT EXISTS idx_chat_sessions_project ON chat_sessions(project_path)'
            )
            cursor.execute(
                'CREATE INDEX IF NOT EXISTS idx_chat_sessions_time ON chat_sessions(last_activity)'
            )
            cursor.execute(
    ...
    def import_claude_logs(self, claude_data_path: str | None = None) -> int:
        """
        ...
        try:
            ...
            monitor = ClaudeMonitorIntegration(
                claude_data_path=claude_data_path, plan_type=self.settings.claude_plan
            )
    ...
    def get_usage_stats(
        ...
        with sqlite3.connect(str(self.db_path), timeout=30.0) as conn:
            ...
            month_conditions = conditions + [
                "DATE(timestamp) >= DATE('now', 'start of month')"
            ]
            cursor.execute(
                f'''
                SELECT 
    ...
    def get_favorite_prompts(
        self, project_path: str | None = None, tags: list[str] | None = None
    ) -> list[FavoritePrompt]:
        """
    ...
    def remove_favorite_prompt(self, prompt_id: str) -> bool:
        """Remove a favorite prompt. Returns True if a row was deleted."""
        with sqlite3.connect(str(self.db_path), timeout=30.0) as conn:
            cursor = conn.cursor()
    ...
    def sync_chat_history(self, project_path: str | None = None) -> int:
        ...
        try:
            ...
            logger.info(
                f"Synced {synced_messages} chat messages from {len(sessions)} sessions"
            )
    ...
    def get_chat_sessions(
        self, project_path: str | None = None, days: int = 30
    ) -> list[dict[str, Any]]:
        """
    ...
    def cleanup_old_data(self, days: int | None = None):
        ...
        logger.info(
            f"Cleaned up {deleted_usage} usage records, {deleted_messages} chat messages, {deleted_sessions} sessions"
        )
    ...
    def get_storage_info(self) -> dict[str, Any]:
        ...
        return {
            ...
            'usage_percentage': round(
                (total_size / (1024 * 1024)) / self.settings.max_storage_mb * 100, 1
            ),
        }
    ...
    def export_data(self, output_path: str, format: str = 'json') -> bool:
        ...
        try:
            with sqlite3.connect(str(self.db_path), timeout=30.0) as conn:
                ...
                # Export favorites
                cursor.execute(
                    'SELECT * FROM favorite_prompts ORDER BY created_at DESC'
                )
                columns = [desc[0] for desc in cursor.description]
                for row in cursor.fetchall():
