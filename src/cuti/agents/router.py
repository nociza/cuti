...
from .base import AgentExecutionContext, BaseAgent
...
class TaskRouter:
    ...
    async def route_task(self, prompt: QueuedPrompt) -> RoutingDecision | None:
    ...
    async def _route_by_capability(self, prompt: QueuedPrompt) -> RoutingDecision | None:
    ...
    async def _route_load_balanced(self, prompt: QueuedPrompt) -> RoutingDecision | None:
    ...
    async def _route_cost_optimized(self, prompt: QueuedPrompt) -> RoutingDecision | None:
    ...
    async def _route_speed_optimized(self, prompt: QueuedPrompt) -> RoutingDecision | None:
        ...
        agent_times: list[tuple[BaseAgent, float]] = []
        for agent in available_agents:
            estimated_time = await agent.estimate_execution_time(prompt)
            load = agent.get_current_load()
            # Adjust time based on current load
            adjusted_time = estimated_time * (1 + load) if estimated_time else None
    ...
    async def _route_quality_optimized(self, prompt: QueuedPrompt) -> RoutingDecision | None:
    ...
    def _record_routing(self, prompt: QueuedPrompt, decision: RoutingDecision) -> None:
...
class CoordinationEngine:
    ...
    async def _execute_sequential(
        ...
        if not agent_names:
            ...
            agents_to_use = [
                agent for name in agent_names
                if (agent := self.agent_pool.get_agent(name)) is not None
            ]
        ...
        for agent in agents_to_use:
            context = AgentExecutionContext(
                ...
                shared_memory=cast(
                    dict[str, Any],
                    await self.memory_manager.get_context(session_id),
                ),
                available_tools=[],
                coordination_data={
