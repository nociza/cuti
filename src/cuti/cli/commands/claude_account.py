...
from ...services.claude_account_manager import ClaudeAccountManager, CredentialType
...
def add_api_key(
    ...
    region: str | None = typer.Option(None, "--region", "-r", help="AWS region (for Bedrock)"),
    access_key: str | None = typer.Option(None, "--access-key", help="AWS access key ID (Bedrock with access keys)"),
    secret_key: str | None = typer.Option(None, "--secret-key", help="AWS secret access key (Bedrock with access keys)"),
    ...
    if provider == "anthropic":
        cred_type = "anthropic_api"
    else:
        cred_type = "bedrock_api"
...
@app.command("env")
def show_env(
    account: str | None = typer.Argument(None, help="Account name (default: active account)"),
