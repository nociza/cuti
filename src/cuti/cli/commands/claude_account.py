...
app = typer.Typer(name="claude", help="Manage Claude accounts and API keys")
...
def list_accounts(
    ...
    backups: bool = typer.Option(
        False, "--backups", help="Include auto-generated backup accounts"
    ),
):
    ...
    for account in accounts:
        ...
        table.add_row(account["name"], account["type"], created, last_used, status)
    ...
    if active:
        ...
        console.print(
            "[dim]Use 'cuti claude use <account>' to activate an account[/dim]"
        )


@app.command("use")
def use_account(account: str = typer.Argument(..., help="Account name to use")):
...
def save_account(
    ...
    force: bool = typer.Option(
        False, "--force", "-f", help="Overwrite existing account"
    ),
):
    ...
    try:
        ...
        console.print(
            f"[dim]Switch to this account anytime with:[/dim] [cyan]cuti claude use \"{name}\"[/cyan]"
        )
    except ValueError as e:
        console.print(f"[red]✗[/red] {str(e)}")
        raise typer.Exit(1)
...
def new_account():
    ...
    try:
        ...
        console.print(
            "  2. Start the Claude setup flow: [cyan]cuti providers auth claude --login[/cyan]"
        )
        console.print("  3. Follow the browser authentication flow")
        console.print(
            "  4. Exit and save: [cyan]cuti claude save -n \"Account Name\"[/cyan]"
        )
        console.print(
...
def delete_account(
    ...
    if not force:
        console.print(
            f"[yellow]⚠[/yellow]  About to delete account: [cyan]{account}[/cyan]"
        )
        if info["is_active"]:
    ...
    try:
        ...
        if info["is_active"]:
            ...
            console.print(
                "[dim]Use 'cuti claude use <account>' to activate another account[/dim]"
            )
    except ValueError as e:
        console.print(f"[red]✗[/red] {str(e)}")
        raise typer.Exit(1)
...
def account_info(
    ...
    content.append(
        f"[bold]Has Credentials:[/bold] {'Yes' if info['has_credentials'] else 'No'}"
    )
    ...
    panel = Panel("\n".join(content), title=f"Account: {account}", border_style="cyan")
    console.print(panel)
...
def current_account():
    ...
    if not active:
        ...
        console.print(
            "  • [cyan]cuti claude use <account>[/cyan] - Activate an account"
        )
...
def add_api_key(
    ...
    api_key: str = typer.Option(
        ...
        "--api-key",
        "-k",
        help="API key or bearer token. Prompted securely (hidden) if omitted, so "
        "the secret stays out of your shell history and process list.",
        prompt="API key or bearer token",
        hide_input=True,
    ),
    provider: str = typer.Option(
        "anthropic", "--provider", "-p", help="Provider: anthropic or bedrock"
    ),
    region: str = typer.Option(None, "--region", "-r", help="AWS region (for Bedrock)"),
    access_key: str = typer.Option(
        None, "--access-key", help="AWS access key ID (Bedrock with access keys)"
    ),
    secret_key: str = typer.Option(
        None, "--secret-key", help="AWS secret access key (Bedrock with access keys)"
    ),
    use_bearer: bool = typer.Option(
        True,
        "--bearer/--access-keys",
        help="Use bearer token (default) or access keys for Bedrock",
    ),
):
    ...
    try:
        if provider == "bedrock":
            if use_bearer:
                ...
                console.print(
                    f"[green]✓[/green] Added Bedrock bearer token to account: [cyan]{account}[/cyan]"
                )
                ...
                if not all([region, access_key, secret_key]):
                    console.print(
                        "[red]✗[/red] Access keys mode requires --region, --access-key, and --secret-key"
                    )
                    raise typer.Exit(1)
                ...
                console.print(
                    f"[green]✓[/green] Added Bedrock access keys to account: [cyan]{account}[/cyan]"
                )
                console.print(f"[dim]Region:[/dim] {region}")
        else:
            manager.save_api_key(account, api_key, provider=cred_type)
            console.print(
                f"[green]✓[/green] Added {provider} API key to account: [cyan]{account}[/cyan]"
            )
...
def show_api_key(
    ...
    show_secret: bool = typer.Option(
        False, "--show-secret", help="Show full API key (default: masked)"
    ),
):
    ...
    if not api_key_info:
        console.print(
            f"[yellow]⚠[/yellow]  No {provider} API key found for account: {account}"
        )
        console.print(
    ...
    if provider == "anthropic":
        ...
        if auth_method == "bearer_token":
            ...
            if show_secret:
                ...
                masked = (
                    bearer_token[:12] + "..." + bearer_token[-6:]
                    if len(bearer_token) > 18
                    else "***"
                )
            ...
            if show_secret:
                ...
                content.append("[bold]Secret Access Key:[/bold] ***")
...
def list_api_keys_cmd(account: str = typer.Argument(..., help="Account name")):
    ...
    if not providers:
        ...
        console.print(
            f"  [cyan]cuti claude add-api-key {account} --api-key YOUR_KEY --provider anthropic[/cyan]"
        )
        return
...
def delete_api_key_cmd(
    account: str = typer.Argument(..., help="Account name"),
    provider: str = typer.Option(
        ..., "--provider", "-p", help="Provider: anthropic or bedrock"
    ),
    force: bool = typer.Option(False, "--force", "-f", help="Skip confirmation"),
):
    ...
    if not force:
        console.print(
            f"[yellow]⚠[/yellow]  About to delete {provider} API key from account: [cyan]{account}[/cyan]"
        )
        confirm = typer.confirm("Are you sure?")
        if not confirm:
            console.print("[dim]Deletion cancelled[/dim]")
    ...
    try:
        if manager.delete_api_key(account, provider):
            console.print(
                f"[green]✓[/green] Deleted {provider} API key from account: [cyan]{account}[/cyan]"
            )
        else:
            console.print("[red]✗[/red] Failed to delete API key")
            raise typer.Exit(1)
    except Exception as e:
...
def update_claude_cli(
    rebuild: bool = typer.Option(
        False, "--rebuild", help="Rebuild the cuti container image first"
    ),
):
...
def show_env(
    ...
    unset: bool = typer.Option(
        False, "--unset", "-u", help="Show unset commands instead"
    ),
):
    ...
    if not account:
        ...
        if not account:
            console.print("[yellow]No active account[/yellow]")
            console.print(
                "[dim]Specify an account: [cyan]cuti claude env <account>[/cyan][/dim]"
            )
            raise typer.Exit(1)
    ...
    if save:
        ...
        if script_file:
            console.print(
                f"[green]✓[/green] Saved environment script to: [cyan]{script_file}[/cyan]"
            )
    ...
    console.print(
        f"[green]Environment variables for account:[/green] [cyan]{account}[/cyan]\n"
    )
    ...
    console.print("  [cyan]eval $(cuti claude env --unset)[/cyan]")
