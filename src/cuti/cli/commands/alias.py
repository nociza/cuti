...
def create_alias(
    ...
    if success:
        ...
        rprint("[red]✗[/red] Failed to create alias (may already exist)")
        raise typer.Exit(1)
...
def update_alias(
    ...
    prompt: str | None = typer.Option(None, "-p", "--prompt", help="New prompt content"),
    description: str | None = typer.Option(None, "-d", "--description", help="New description"),
    working_dir: str | None = typer.Option(None, "-w", "--working-dir", help="New working directory"),
    storage_dir: str = typer.Option("~/.cuti", help="Storage directory"),
) -> None:
