...
def list(project, tags, limit):
    ...
    # Get favorites
    favorites = manager.get_favorite_prompts(project_path=project, tags=tag_list)[
        :limit
    ]
...
def add(title, content, file, tags, project):
    ...
    favorite_id = manager.add_favorite_prompt(
        title=title, content=content, tags=tag_list, project_path=project or os.getcwd()
    )
...
def remove(favorite_id):
    ...
    if click.confirm(f"Remove favorite '{favorite_id}'?"):
        if manager.remove_favorite_prompt(favorite_id):
            console.print(f"[green]✓[/green] Removed favorite '{favorite_id}'")
        else:
            console.print(f"[yellow]No favorite found with id '{favorite_id}'[/yellow]")
...
def search(project, tags):
    ...
    favorites = manager.get_favorite_prompts(project_path=project_path, tags=tag_list)
