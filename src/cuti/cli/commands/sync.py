...
from ...services.container_usage_sync import (
    get_container_sync,
)
from ...services.container_usage_sync import (
    sync_now as container_sync_now,
)
...
def status():
    ...
    table.add_row(
        "Tracking Enabled", "✅ Yes" if status['tracking_enabled'] else "❌ No"
    )
...
def chat(
    ...
    project_path: str | None = typer.Option(
        None, "--project", "-p", help="Project path to sync"
    ),
):
...
def history(
    ...
    session_id: str | None = typer.Option(
        None, "--session", "-s", help="Specific session ID"
    ),
    ...
    # Get chat history
    messages = manager.get_chat_history(session_id=session_id, days=days, limit=limit)
