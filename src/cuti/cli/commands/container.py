        ...
        for line in result.stdout.strip().split('\n'):
            ...
            try:
                ...
                if inspect_result.returncode == 0:
                    ...
                    container_mode = labels.get("cuti.container_mode", "unknown")
                ...
                console.print(
                    f"[yellow]Warning: Could not parse container info: {e}[/yellow]"
                )
                continue
...
def status(
        ...
        for container in container_list:
            if verbose:
                table.add_row(
                    ...
                    container.get('container_mode', 'unknown'),
                    container['created'],
    ...
    console.print(
        "[dim]• Enter a container: docker exec -it <container-id> /bin/zsh[/dim]"
    )
    console.print(
        "[dim]• Stop all: docker stop $(docker ps -q --filter 'ancestor=cuti-dev-universal')[/dim]"
    )
...
def stop(
    ...
    if all:
        ...
        if not force:
            console.print(
                f"[yellow]This will stop {total_containers} container(s) across {len(containers)} workspace(s)[/yellow]"
            )
        ...
        # Stop all containers
        result = subprocess.run(
            ["docker", "stop"] + all_ids, capture_output=True, text=True
        )

        if result.returncode == 0:
            console.print(
                f"[green]✅ Successfully stopped {total_containers} container(s)[/green]"
            )
        else:
        ...
        if not container_id and is_running_in_container():
            ...
            if container_id:
                console.print(
                    f"[cyan]Stopping current container {container_id}...[/cyan]"
                )
...
def enter(
    ...
    if workspace:
        ...
        if len(all_containers) == 1:
            ...
            console.print(
                "[yellow]Multiple containers running. Please specify one:[/yellow]"
            )
            for ws_path, container_list in containers.items():
                for container in container_list:
...
def cleanup(
    ...
    if not force:
        console.print(
            f"[yellow]This will remove {len(to_remove)} stopped container(s)[/yellow]"
        )
        if not typer.confirm("Continue?"):
            raise typer.Abort()

    result = subprocess.run(
        ["docker", "rm"] + to_remove, capture_output=True, text=True
    )

    if result.returncode == 0:
        console.print(
            f"[green]✅ Removed {len(to_remove)} stopped container(s)[/green]"
        )
    else:
