    ...
    try:
        ...
        for i, todo in enumerate(sub_list.todos, 1):
            ...
            if 'linked_goal_id' in todo.metadata:
                print(
                    f"     → Linked to: {todo.metadata.get('goal_content', 'N/A')[:40]}..."
                )
...
def _run_hierarchical_structure():
    ...
    try:
        ...
        print(
            f"  - Completed: {sum(1 for t in master_list.todos if t.status == TodoStatus.COMPLETED)}"
        )
        print(
            f"  - Pending: {sum(1 for t in master_list.todos if t.status == TodoStatus.PENDING)}"
        )
...
def _run_todo_operations():
    ...
    try:
        ...
        success = todo_service.update_todo(
            test_todo.id, {"status": TodoStatus.IN_PROGRESS}
        )
...
def test_goal_sync():
    """Test syncing GOAL.md with master todo list."""
    assert _run_goal_sync()


def test_claude_sync():
    """Test Claude todo capture functionality."""
    assert _run_claude_sync()


def test_hierarchical_structure():
    """Test the hierarchical todo structure."""
    assert _run_hierarchical_structure()


def test_todo_operations():
    """Test basic todo CRUD operations."""
    assert _run_todo_operations()
...
def main():
    ...
    # Run tests
    results.append(("GOAL.md Sync", _run_goal_sync()))
    results.append(("Claude Todo Capture", _run_claude_sync()))
    results.append(("Hierarchical Structure", _run_hierarchical_structure()))
    results.append(("Todo Operations", _run_todo_operations()))
